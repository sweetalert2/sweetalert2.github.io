import { c as commonjsGlobal, g as getDefaultExportFromCjs, R as React, a as createRoot, j as jsxRuntimeExports, r as reactExports, b as client } from "./client-d8a4dc41.js";
const styles = "";
  ...
  (function(global, factory) {
    ...
    const applyCustomClass = (elem, params, className) => {
      ...
      if (params.customClass && params.customClass[className]) {
        if (typeof params.customClass[className] !== "string" && !params.customClass[className].forEach) {
          warn2("Invalid type of customClass.".concat(className, '! Expected string or iterable object, got "').concat(typeof params.customClass[className], '"'));
          return;
        }
    ...
    const showInput = (params) => {
      ...
      if (!renderInputType[params.input]) {
        error2('Unexpected type of input! Expected "text", "email", "password", "number", "tel", "select", "radio", "checkbox", "textarea", "file" or "url", got "'.concat(params.input, '"'));
        return;
      }
...
const functions = {
  ...
  async fileInput() {
    ...
    if (file) {
      ...
      reader.onload = (e2) => {
        var _a;
        const imageUrl = (_a = e2.target) == null ? void 0 : _a.result;
...
Object.entries(functions).forEach(([id, fn2]) => {
  functionStrings[id] = String(fn2).replace(/import_sweetalert\d+.default/g, "Swal").replace(/^.*\n/, "").replace(/}$/, "").replace(/(^|\n) {4}/g, "$1").replace(/.*rtl-container.*\n/, "").replace(/(\d)e3/g, "$1000").replace(/\/\*!/g, "/*").replace(/\\u([\d\w]{4})/gi, function(_2, grp) {
